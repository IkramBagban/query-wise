...
export function PublicShareView({ token }: { token: string }) {
  ...
  return (
    <main className="mx-auto max-w-7xl space-y-6 p-4 sm:p-8">
      ...
      <div className="grid auto-rows-[18rem] gap-4 lg:grid-cols-2">
        {data.dashboard.widgets.map((widget) => (
          <Card key={widget.id} className="flex h-full flex-col overflow-hidden">
            <h2 className="border-b border-border px-4 py-3 font-medium">{widget.title}</h2>
