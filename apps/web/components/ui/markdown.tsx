...
export function Markdown({ children, className = "" }: { children: string; className?: string }) {
  return (
    <div className={`min-w-0 text-sm leading-relaxed text-text-1 ${className}`}>
      <ReactMarkdown
        ...
        components={{
          ...
          a: ({ href, children }) => (
            <a href={href} target="_blank" rel="noreferrer" className="text-accent-2 underline underline-offset-2">{children}</a>
          ),
          ...
          th: ({ children }) => <th className="border-b border-border px-3 py-2 font-semibold text-text-2">{children}</th>,
          td: ({ children }) => <td className="border-b border-border px-3 py-1.5 tabular-nums last:border-b-0">{children}</td>,
          hr: () => <hr className="my-3 border-border" />,
          blockquote: ({ children }) => (
            <blockquote className="my-2 border-l-2 border-accent-2/40 pl-3 text-text-2">{children}</blockquote>
          ),
        }}
      >
        {children}
