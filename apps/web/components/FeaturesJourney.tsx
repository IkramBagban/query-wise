...
export default function FeaturesJourney() {
  ...
  // 7 items -> 8 segments of scroll to give the last item dwell time
  ...
  // The line finishes filling exactly when the active index hits the last item (at 6/7 of the total scroll)
  ...
  return (
    <section id="features" data-screen-label="Features" style={{ background: 'var(--bg)' }}>
      {/* Increased height to 800vh to add dwell time for the last item */}
      <div ref={containerRef} style={{ height: '800vh', position: 'relative' }}>
        
        {/* Sticky inner container - Reduced padding to prevent content from overflowing on smaller laptop screens */}
        <div style={{ position: 'sticky', top: 0, height: '100vh', display: 'flex', flexDirection: 'column', padding: '60px 28px', overflow: 'hidden' }}>
          <div style={{ maxWidth: '1180px', margin: '0 auto', width: '100%', display: 'flex', flexDirection: 'column', height: '100%' }}>
            ...
            <div data-reveal style={{ maxWidth: '640px', flexShrink: 0 }}>
              <p style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: '11px', letterSpacing: '0.18em', color: 'var(--accent)', margin: '0 0 10px' }}>FEATURES</p>
            ...
            {/* Main Grid container - tightened gaps to fit smaller viewports */}
            <div data-reveal style={{ display: 'grid', gridTemplateColumns: '290px 1fr', gap: '32px', marginTop: '30px', alignItems: 'stretch', flex: 1, minHeight: 0 }}>
              
              {/* Left Column: Journey Navigation */}
              <div style={{ position: 'relative', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', paddingBottom: '10px' }}>
                {/* Journey Line Background - Positioned exactly at icon centers (10px padding + 16px half icon = 26px) */}
                <div style={{ position: 'absolute', left: '26px', top: '26px', bottom: '26px', width: '2px', background: 'var(--border)' }} />
                
                {/* Journey Line Fill (Animated) */}
                <motion.div 
                  style={{ 
                    position: 'absolute', left: '26px', top: '26px', bottom: '26px', width: '2px', 
                  ...
                  return (
                    ...
                    >
                      ...
                      <span style={{ display: 'flex', flexDirection: 'column', gap: '2px', minWidth: 0 }}>
                        <motion.span animate={{ color: on ? 'var(--text)' : 'var(--muted)' }} style={{ fontSize: '14px', fontWeight: 600 }}>{f.title}</motion.span>
                        <span style={{ fontSize: '11.5px', color: 'var(--faint)', lineHeight: 1.3 }}>{f.tag}</span>
                      </span>
                    </button>
                  );
                })}
              ...
              {/* Right Column: Rich Visual Panel */}
              <div style={{ position: 'relative', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '18px', boxShadow: 'var(--shadow)', overflow: 'hidden', height: '100%', display: 'flex', flexDirection: 'column' }}>
                ...
                {/* Properly centered wrapper for BOTH text and visuals, packed tight so it never overflows */}
                <div style={{ flex: 1, padding: '24px 30px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
...
// Subcomponents for each step with micro-animations
...
function SqlVisual() {
  ...
  return (
    <div style={{ width: '100%', maxWidth: '460px', background: 'var(--code-bg)', border: '1px solid var(--border)', borderRadius: '14px', padding: '20px 22px' }}>
      ...
      <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: '13px', lineHeight: 1.85, whiteSpace: 'pre-wrap' }}>
        {codeLines.map((tok, i) => (
          <motion.span
            key={i}
            initial={{ opacity: 0 }}
...
function ChartVisual() {
  const [chartType, setChartType] = useState(0); // 0: Bar, 1: Line, 2: Area, 3: Pie
      ...
      <div style={{ display: 'flex', gap: '6px', marginBottom: '30px' }}>
        {types.map((t, i) => (
          <motion.span
            key={t}
            animate={{ 
              backgroundColor: chartType === i ? 'var(--accent)' : 'transparent',
              color: chartType === i ? 'var(--accent-ink)' : 'var(--muted)',
              borderColor: chartType === i ? 'transparent' : 'var(--border)'
            }}
