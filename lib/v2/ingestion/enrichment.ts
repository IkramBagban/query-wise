...
import { z } from "zod";
import type { MetadataEntity } from "@/types/v2";
import { getModel, type Provider, withModelFallback } from "@/lib/llm/client";
import type { SchemaEmbeddingRecord, SchemaEntityDescription } from "./types";
...
function configuredProvider(): { provider: Provider; model: string; apiKey: string } | null {
  const provider = process.env.QUERYWISE_INGESTION_LLM_PROVIDER as Provider | undefined;
  const model = process.env.QUERYWISE_INGESTION_LLM_MODEL;
  const apiKey = process.env.QUERYWISE_INGESTION_LLM_API_KEY;
  if ((provider === "google" || provider === "anthropic") && model && apiKey) {
...
async function describeBatchWithLlm(entities: MetadataEntity[], config: { provider: Provider; model: string; apiKey: string }) {
  ...
  const parsed = descriptionSchema.safeParse(JSON.parse(jsonText));
  if (!parsed.success) {
