...
export const connectionsApi = {
  ...
  update: (id: string, input: { name?: string; credential?: { connectionString: string } }) =>
...
export const conversationsApi = {
  ...
  messages: (id: string, limit = 100, cursor?: string) =>
    apiRequest<CursorPage<MessageDto>>(`/api/conversations/${id}/messages`, {
      query: pageQuery(limit, cursor),
    }),
  submit: (input: Omit<SubmitQueryInput, "idempotencyKey">) =>
    apiRequest<QueryAccepted>("/api/query", {
      method: "POST",
      body: { ...input, idempotencyKey: createIdempotencyKey() },
    }),
  queryRun: (id: string) => apiRequest<QueryRunDto>(`/api/query-runs/${id}`),
};
...
export const dashboardsApi = {
  ...
  shares: (id: string) => apiRequest<CursorPage<ShareListItem>>(`/api/dashboards/${id}/shares`),
  createShare: (id: string, input: { password?: string; recipientEmail?: string; expiresAt?: string | null }) =>
    apiRequest<ShareListItem>(`/api/dashboards/${id}/shares`, {
      method: "POST",
      body: { ...input, idempotencyKey: createIdempotencyKey() },
    }),
...
export const publicSharesApi = {
  get: (token: string, unlockCredential?: string) =>
    apiRequest<PublicDashboardDto>(`/api/public/shares/${token}`, {
      headers: unlockCredential ? { Authorization: `Share ${unlockCredential}` } : undefined,
    }),
  unlock: (token: string, password: string) =>
    apiRequest<{ unlockCredential: string }>(`/api/public/shares/${token}/unlock`, {
      method: "POST",
      body: { password },
    }),
};
